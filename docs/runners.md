...
Fedora disables sudo's PAM account-validation phase only for the disposable `validator` user, whose explicit NOPASSWD sudoers rule still controls access. Fedora's PAM account check fails in this Ubuntu-hosted runtime even though the same image passes during build. The applied sudoers configuration is retained as an artifact. OS account authentication is outside this validator's coverage.
